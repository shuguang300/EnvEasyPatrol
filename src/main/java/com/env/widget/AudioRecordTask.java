...
public class AudioRecordTask extends LinearLayout{
	...
	public AudioRecordTask(Context context,HashMap<String, String> task){
		super(context);
		LayoutInflater.from(context).inflate(R.layout.audio_record_task, this);
		this.context = context;
		this.task = task;
		ini();
	}
	
	public AudioRecordTask(Context context, AttributeSet attrs) {
		super(context, attrs);
		LayoutInflater.from(context).inflate(R.layout.audio_record_task, this);
		this.context = context;
		ini();
	...
	private void ini(){
		statu = (ImageView)findViewById(R.id.audio_record_task_logo);
		taskName = (TextView)findViewById(R.id.audio_record_task_name);
		taskTime = (TextView)findViewById(R.id.audio_record_task_time);
		dataCount = (TextView)findViewById(R.id.audio_record_task_count);
		rootView = (RelativeLayout)findViewById(R.id.audio_record_task_root);
	}
	...
	public void setClickEvent(int requsetCode,int pos){
		request = requsetCode;
		rootView.setClickable(true);
		if(pos%2==1){
			rootView.setBackgroundResource(R.drawable.single_click_middle_gray_stroke);
